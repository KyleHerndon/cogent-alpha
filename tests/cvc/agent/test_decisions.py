...
def _make_hub(x=44, y=44):
    return KnownEntity(
        entity_type="hub", global_x=x, global_y=y,
        labels=(), team="team_0", owner="team_0",
        last_seen_step=100, attributes={},
    )


def _make_ctx(
    *,
    hp=100, step=500, hub_distance=0, hub=None,
    hearts=0, cargo=0, stalled_steps=0, oscillation_steps=0,
    in_enemy_aoe=False, near_enemy_territory=False,
    state=None,
):
...
class TestCheckWipeoutRecovery:
    ...
    def test_skips_no_hub(self):
        ...
        ctx = TickContext(
            state=ctx.state, position=ctx.position, hp=0, step=ctx.step,
            team=ctx.team, resource_bias=ctx.resource_bias, hearts=ctx.hearts,
            cargo=ctx.cargo, hub=None, hub_distance=0,
            in_enemy_aoe=False, near_enemy_territory=False,
            friendly_junctions=[], enemy_junctions=[], neutral_junctions=[],
            network_sources=[], stalled_steps=0, oscillation_steps=0,
            teammate_aligner_positions=[],
        )
        assert check_wipeout_recovery(ctx, "miner", _make_engine()) is None
